...
# feat_path = '../flickr8k/196x512_perfile_dense_name/'
...
def prepare_data(caps, features, worddict, maxlen=None, n_words=10000, zero_pad=False):
    ...
    for cc in caps:
        ...
        feat_list.append(loadmat(feat_path + str(cc[1]) + '.mat')['feats'])
    ...
    y = numpy.array(feat_list).reshape([len(feat_list), 14*14, 512]).astype('float32')
    if zero_pad:
...
def load_data(load_train=True, load_dev=True, load_test=True, path='../flickr8k/'):
    ''' Loads the dataset
    ...
    if load_train:
        train_cap = pkl.load(open(path+'cap_train.pkl', 'rb'))
        train_feat = []
        train = (train_cap, train_feat)
        # train = (reverse_cap(train_cap), train_feat)
    else:
        train = None
    if load_test:
        test_cap = pkl.load(open(path+'cap_test.pkl', 'rb'))
        test_feat = []
        test = (test_cap, test_feat)
        # test = (reverse_cap(test_cap), test_feat)
    else:
        test = None
    if load_dev:
        dev_cap = pkl.load(open(path+'cap_dev.pkl', 'rb'))
        dev_feat = []
        valid = (dev_cap, dev_feat)
        # valid = (reverse_cap(dev_cap), dev_feat)
    else:
        valid = None
